...
pub fn spawn_terminal(
    ...
    // can call OrchestraAI's MCP server. `id` is the terminalId — a random UUID
    // that only this shell's env sees — and doubles as the unguessable bearer
    // token authorising the MCP call. `ORCHESTRAAI_MCP_URL` points at the
    // Streamable-HTTP endpoint bound at app boot; if the server failed to
    // bind, we skip the URL var (agents just won't have the orchestraai server
    // available) but still set the session so future subsystems can use it.
    cmd.env("ORCHESTRAAI_SESSION", &id);
    if let Some(url) = state.mcp_url.get() {
        cmd.env("ORCHESTRAAI_MCP_URL", url.as_ref());
    }
